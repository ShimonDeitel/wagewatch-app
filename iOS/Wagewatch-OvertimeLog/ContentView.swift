...
struct ContentView: View {
    ...
    @State private var editingItem: Shift?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Wagewatch - Overtime Log")
            .toolbar {
    ...
    @ViewBuilder
    func row(for item: Shift) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(item.title)
                .font(Theme.headlineFont)
                .foregroundStyle(Theme.ink)
            Text("\(String(describing: item.employee))  \u{00B7}  \(String(describing: item.hours))")
                .font(.caption)
...
struct ItemEditorView: View {
    ...
    init(mode: EditorMode, onSave: @escaping (Shift) -> Void) {
        self.mode = mode
        self.onSave = onSave
        switch mode {
        case .add:
            _draft = State(initialValue: Shift(title: ""))
        case .edit(let item):
        ...
        NavigationStack {
            Form {
                Section("Shift") {
                    ...
                    TextField("Employee", text: $draft.employee)
                        .accessibilityIdentifier("field.employee")
            ...
            .navigationTitle(mode == .add ? "Add Shift" : "Edit Shift")
            .toolbar {
