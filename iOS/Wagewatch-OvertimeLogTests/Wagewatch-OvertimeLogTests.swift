import XCTest
@testable import Wagewatch-OvertimeLog

@MainActor
final class Wagewatch-OvertimeLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        let item = Shift(title: "Test Item")
    ...
    func testAddRespectsFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Shift(title: "Item \(i)"), isPro: false)
        }
        let added = store.add(Shift(title: "Overflow"), isPro: false)
    ...
    func testProBypassesFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Shift(title: "Item \(i)"), isPro: true)
        }
        let added = store.add(Shift(title: "Extra"), isPro: true)
        XCTAssertTrue(added)
        XCTAssertEqual(store.items.count, Store.freeLimit + 1)
    }

    func testDeleteByIdRemovesItem() {
        let item = Shift(title: "Delete Me")
        _ = store.add(item, isPro: false)
    ...
    func testPersistenceRoundTrip() {
        let item = Shift(title: "Persisted")
        _ = store.add(item, isPro: false)
        let reloaded = Store()
